...
stdEnv :: Env
stdEnv =  Env { fp = "", be = C, runs = 10 }

benchmark :: LFun -> Val -> Backend -> Runs -> IO (CommandExecution Result)
benchmark futPgmStr val backend runs =
  let futPgmStr2 = completeCodeGen futPgmStr val
   in execCmd (benchmarkM futPgmStr2 val) <| stdEnv { be = backend, runs = runs }

benchmarkM :: FutPgmStr -> Val -> Command Result
benchmarkM futPgmStr val = do
  filepath <- store <| makeHeader val ++ futPgmStr
  env <- ask
  let subEnv = env { fp = filepath }
...
runBenchmark = do
  ...
  -- See: https://futhark.readthedocs.io/en/stable/man/futhark-bench.html#futhark-bench-1
  let { params =
        [ "bench"
        , "--json=" ++ jsonfile
        , "--runs=" ++ show runs
